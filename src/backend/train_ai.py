...
class TensorBoardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.step % 1000 == 0:  # Log every 1000 steps
            ...
            logging.info(
                f"Step {self.step}: Mean reward = {np.mean(self.locals['rewards']):.2f}"
            )
        return True
...
def train_ai(total_timesteps=100000, learning_rate=0.0003, n_steps=2048):
    logging.info(
        f"Starting AI training with {total_timesteps} timesteps, "
        f"learning rate {learning_rate}, and {n_steps} steps per update"
    )
...
def start_tensorboard():
    ...
    logging.info(
        "TensorBoard started. Open http://localhost:6006/ in your browser to view."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train and evaluate an AI for the Board Game Gym"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--test_episodes", type=int, default=5, help="Number of episodes for testing"
    )
    args = parser.parse_args()

    logging.info("Starting AI training process")
    model, env = train_ai(
        total_timesteps=args.timesteps, learning_rate=args.lr, n_steps=args.n_steps
    )
